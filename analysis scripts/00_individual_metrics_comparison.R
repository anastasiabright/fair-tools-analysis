...
# ggplot(fuji_fair_long, aes(Var1, Var2, fill = value)) +
#   geom_tile() +
#   scale_fill_gradient(low = "lightblue", high = "darkblue") +
#   theme_minimal() +
#   labs(title = "Heatmap of Fuji Percent F1 vs Fair Checker F1",
#        x = "Fuji Percent F1",
#        y = "Fair Checker F1")

# intersect_f1$fuji_percent_f1 <- as.factor(intersect_f1$fuji_percent_f1)
# intersect_f1$fair_checker_f1 <- as.factor(intersect_f1$fair_checker_f1)
# intersect_f1$enough_f1 <- as.factor(intersect_f1$enough_f1)
# contingency_table <- table(intersect_f1$fuji_percent_f1, 
#                            intersect_f1$fair_checker_f1, 
#                            intersect_f1$enough_f1)
# t.test(intersect_f1$fuji_percent_f1, intersect_f1$fair_checker_f1, paired = TRUE)
# t.test(intersect_f1$fair_checker_f1, intersect_f1$enough_f1, paired = TRUE)
# t.test(intersect_f1$fuji_percent_f1, intersect_f1$enough_f1, paired = TRUE)
# 
# 
# wilcox.test(intersect_f1$fuji_percent_f1, intersect_f1$fair_checker_f1, paired = TRUE)
# wilcox.test(intersect_f1$fair_checker_f1, intersect_f1$enough_f1, paired = TRUE)
# wilcox.test(intersect_f1$fuji_percent_f1, intersect_f1$enough_f1, paired = TRUE)

#+
#+ Findability
#+
# selected_articles <- selected_articles |>
#   mutate(enough_f = round((fair_enough_f1 + fair_enough_f2+fair_enough_f3+ fair_enough_f4)/8), digits = 2)
# 
# f <- selected_articles |>
#   select(article, id, repository, repository_type,
#         fuji_percent_f, enough_f, checker_percent_f)
# 
# tolerance <- 20
# 
# f <- f |>
#   mutate(equal_all = abs(fuji_percent_f - enough_f) <= tolerance &
#            abs(fuji_percent_f - checker_percent_f) <= tolerance &
#            abs(enough_f - checker_percent_f) <= tolerance) |>
#   mutate(fuji_checker_egual = abs(fuji_percent_f - checker_percent_f) <= tolerance,
#          enough_checker_equal = abs(checker_percent_f - enough_f) <= tolerance,
#          fuji_enough_equal = abs(fuji_percent_f - enough_f) <= tolerance)
# 
# fuji_checker_f <- table(f$fuji_percent_f, f$checker_percent_f)
# fuji_checker_f
# enough_checker_f <- table(f$checker_percent_f, f$enough_f)
# enough_checker_f
# fuji_enough_f <- table(f$fuji_percent_f, f$enough_f)
# fuji_enough_f
# 
# fuji_enough_long_f <- melt(as.matrix(fuji_enough_f))#, varnames =  c("FujiPercentF1", "EnoughF1"))
# enough_checker_long_f <- melt(as.matrix(enough_checker_f))#, varnames = c("EnoughF1", "FairCheckerF1"))
# fuji_checker_long_f <- melt(as.matrix(fuji_checker_f))#, varnames = c("FujiPercentF1", "FairCheckerF1"))
# 
# # Add a new column to identify each comparison
# fuji_enough_long_f$Comparison <- "Fuji vs Enough"
# enough_checker_long_f$Comparison <- "Enough vs Fair Checker"
# fuji_checker_long_f$Comparison <- "Fuji vs Fair Checker"
# 
# combined_long_f <- rbind(fuji_enough_long_f, enough_checker_long_f, fuji_checker_long_f)
# 
# # Add a new column to combined_long to indicate equality within tolerance
# combined_long_f <- combined_long_f %>%
#   mutate(equal_within_tolerance = abs(Var1 - Var2) <= tolerance)
# 
# equality_summary_f <- f |>
#   summarise(
#     all_equal = sum(equal_all),
#     fuji_checker_equal = sum(fuji_checker_egual),
#     enough_checker_equal = sum(enough_checker_equal),
#     fuji_enough_equal = sum(fuji_enough_equal)
#   )
# 
# equality_summary_long_f <- gather(equality_summary_f, key = "condition", value = "count")
# 
# 
# f_long <- melt(f, id.vars = "id")
# 
# # Violin plot
# ggplot(combined_long_f, aes(x = Var1, y = value, fill = equal_within_tolerance)) +
#   geom_violin(width = 0.8) +
#   facet_wrap(~ Comparison, scales = "free") +
#   theme_minimal() +
#   labs(title = "Violin Plot of Pairwise Comparisons with Tolerance",
#        x = "Variable 1",
#        y = "Value") +
#   scale_fill_manual(values = c("TRUE" = "blue", "FALSE" = "red"), labels = c("Equal", "Not Equal"))
