...
#load("output/Rdata/joined_all_2025_01_29.Rdata")
...
fuji_missing_enough <- fuji_results_all[!fuji_results_all$best_identifier %in% 
                                          fair_enough_df_full$ID,]
missing_checker <- sum(is.na(fair_checker_summary_wide_full_correct$fair_checker_score)) #7
...
joined_cleaned_long <- joined_cleaned |>
  pivot_longer(cols = c("fuji_percent", 
                        "fair_enough_score_metrics_percent", 
                        "fair_checker_score"),
               names_to = "assessment_tool",
               values_to = "score")

#write.csv2(joined_cleaned, "./output/csv/df_all_tools_joined_cleaned_2024_11_05.csv", row.names = F)
...
ggplot(joined_cleaned_long, aes(x = assessment_tool, 
  ...
  scale_x_discrete(labels = c("fuji_percent" = "FUJI", 
                              "fair_enough_score_metrics_percent" = "FAIR Enough", 
...
#write.csv2(articles_general, "./output/csv/articles_general_all_tools_2025_01_29.csv", row.names = F)

articles_specific <- joined_cleaned |>
  filter(repository_type == "disciplinary repository") # 280

#write.csv2(articles_specific, "./output/csv/articles_disciplinary_all_tools_2024_11_02.csv", row.names = F)
...
save(selected_articles, file = "./output/Rdata/selected_articles_df_2025_01_29.Rdata")
#write.csv2(selected_articles, file = "./output/csv/selected_articles_df_2024_11_02.csv", row.names = F)


fair_enough_overall_vs_indiv <- fair_enough_df_full[fair_enough_df_full$ID %in% 
                                                      selected_articles$id,]
fair_enough_overall_vs_indiv_compare <- fair_enough_overall_vs_indiv |>
  select(c(ID, fair_enough_score_percent, fair_enough_score_metrics_percent))

selected_long <-
  selected_articles |> pivot_longer(cols = c("fuji_percent", 
                                             "fair_enough_score_metrics_percent", 
                                             "fair_checker_score"),
                         names_to = "assessment_tool",
                         values_to = "score")
...
create_assessment_tool_plot <- function(tool_name, data) {
  plot <- ggplot(data, aes(x = repository_type, y = score, fill = repository_type)) +
    ...
    facet_wrap(~assessment_tool, scales = "free_y", nrow = 1)  +
    scale_y_continuous(limits = c(0, 100))
  return(plot)
}

plot_fuji <- create_assessment_tool_plot("FUJI", selected_long[selected_long$assessment_tool == "fuji_percent", ])
plot_enough <- create_assessment_tool_plot("FAIR Enough", selected_long[selected_long$assessment_tool == "fair_enough_score_metrics_percent", ])
...
df_fuji_overlapping <- fuji_cleaned |>
  select(c(article, best_identifier, repository,repository_type, 
           ...
           `fuji_FsF_F1_01D`, `fuji_FsF_F1_02D`, `fuji_FsF_F2_01M`,
           `fuji_FsF_I1_01M`, `fuji_FsF_I2_01M`, `fuji_FsF_I3_01M`, `fuji_FsF_R1.1_01M`
            ))|>
  mutate(fuji_score = round(((`fuji_FsF_F1_01D`+ `fuji_FsF_F1_02D`+`fuji_FsF_F2_01M`+
                            `fuji_FsF_I1_01M`+`fuji_FsF_I2_01M`+`fuji_FsF_I3_01M`+
                              `fuji_FsF_R1.1_01M`)) / 15.5 * 100, digits = 2))
 # 1.5 + 1.5 + 3.5 + 3 + 1 + 2 + 3
...
df_enough_overlapping <- enough_cleaned |>
  select(c(ID,
           ...
           fair_enough_Gen2_FM_F1A, fair_enough_Gen2_FM_F1B_metadata, fair_enough_Gen2_FM_F2A, fair_enough_Gen2_FM_F2B,
           fair_enough_Gen2_FM_I1A_metadata, fair_enough_Gen2_FM_I1B_metadata, 
           fair_enough_Gen2_FM_I1A_data, fair_enough_Gen2_FM_I1B_data, fair_enough_Gen2_FM_I2A, fair_enough_Gen2_FM_I2B,
           fair_enough_Gen2_FM_I3, fair_enough_Gen2_FM_R1.1_metadata, fair_enough_Gen2_FM_R1.1_data)) |>
  mutate(fair_enough_score = round((fair_enough_Gen2_FM_F1B_metadata+fair_enough_Gen2_FM_F1A + 
                                 fair_enough_Gen2_FM_F2A + fair_enough_Gen2_FM_F2B +
                                 fair_enough_Gen2_FM_I1A_metadata+ fair_enough_Gen2_FM_I1B_metadata+
                                 fair_enough_Gen2_FM_I1A_data+ fair_enough_Gen2_FM_I1B_data+ 
                                 fair_enough_Gen2_FM_I2A+ fair_enough_Gen2_FM_I2B+
                                 fair_enough_Gen2_FM_I3+ fair_enough_Gen2_FM_R1.1_metadata+ 
                                 fair_enough_Gen2_FM_R1.1_data)/13*100, digits = 2))

checker_cleaned <- fair_checker_summary_wide_full_correct[fair_checker_summary_wide_full_correct$target_uri %in%
                                                            joined_cleaned$id,]
df_checker_overlapping <- checker_cleaned |>
  select(c(fair_checker_f1, fair_checker_f2, fair_checker_i1, fair_checker_i2, fair_checker_i3, fair_checker_r1.1,
           fair_checker_F1A, fair_checker_F1B, fair_checker_F2A, fair_checker_F2B, 
           fair_checker_I1, fair_checker_I2, fair_checker_I3, fair_checker_R1.1)) |>
  mutate(fair_checker_score = round((fair_checker_F1A+fair_checker_F1B+
                                  fair_checker_F2A+fair_checker_F2B+
                                  fair_checker_I1+fair_checker_I2+fair_checker_I3+
                                  fair_checker_R1.1)/16*100, digits = 2))
...
overlapping_long <-
  df_overlapping |> pivot_longer(cols = c("fuji_score", "fair_enough_score", "fair_checker_score"),
                                 names_to = "assessment_tool",
                                 values_to = "score") |>
  select(-ID)
save(overlapping_long, file = "output/Rdata/all_overlapping_indicators_long_2025_01_29.Rdata")
...
# 
# plot_overlap_enough <- plot_ly(df_overlapping, y = ~fair_enough_score, type = "box", name = "FAIR Enough") |>
#   layout(title = "All data sets with overlapping indicators", yaxis = list(title = "Score in %"),showlegend = FALSE)
# 
# plot_overlap_checker <- plot_ly(df_overlapping, y = ~fair_checker_score, type = "box", name = "FAIR Checker") |>
#   layout(title = "All data sets with overlapping indicators", yaxis = list(title = "Score in %"),showlegend = FALSE)
# 
...
save(selected_articles_overlap, file="./output/csv/selected_articles_overlapping_indicators_2025_01_29.Rdata")
write.csv2(selected_articles_overlap, file="./output/csv/selected_articles_overlapping_indicators_2025_01_29.csv",
           row.names = F)

intersect_overlap_sample <- selected_articles_overlap |>
  mutate(all_equal = (fuji_score == fair_checker_score) & (fair_checker_score == fair_enough_score),#14
         fuji_enough_egual =  (fuji_score == fair_checker_score), # 38
         enough_checker_equal = (fair_checker_score == fair_enough_score)) # 19

overlapping_selected_long <-
  selected_articles_overlap |> pivot_longer(cols = c("fuji_score", "fair_enough_score", "fair_checker_score"),
                                 names_to = "assessment_tool",
                                 values_to = "score")

save(overlapping_selected_long,file="output/Rdata/selected_articles_overlapping_indicators_long_2025_01_29.Rdata")
...
overlap_plot <- ggplot(overlapping_selected_long, aes(x = assessment_tool, 
                                y = score,
                                fill = assessment_tool)) +
  geom_violin(trim = T, 
              colour = "#555555",
  ...
  theme(axis.text.x = element_text(hjust = 0.5),
        legend.position = "bottom") +
  scale_x_discrete(labels = c("fuji_score"="FUJI", 
                              "fair_enough_score" = "FAIR Enough", 
                              "fair_checker_score" = "FAIR Checker")) +
  scale_y_continuous(limits = c(0, 100))
...
overlapping_gen_selected_long <-
  sample_overl_indic_general |> pivot_longer(cols = c("fuji_score", 
                                                      "fair_enough_score", 
                                                      "fair_checker_score"),
                                            names_to = "assessment_tool")
...
plot_overlap_gen_enough <- plot_ly(sample_overl_indic_general, y = ~fair_enough_score, type = "box", name = "FAIR Enough") |>
  layout(title = "Selected data sets with overlapping indicators (general repositories)", yaxis = list(title = "Score in %"),showlegend = FALSE)

plot_overlap_gen_checker <- plot_ly(sample_overl_indic_general, y = ~fair_checker_score, type = "box", name = "FAIR Checker") |>
  layout(title = "Selected data sets with overlapping indicators (general repositories)", yaxis = list(title = "Score in %"),showlegend = FALSE)
...
overlapping_disc_selected_long <-
  sample_overl_indic_specific|> pivot_longer(cols = c("fuji_score", "fair_enough_score", "fair_checker_score"),
                                             names_to = "assessment_tool")
...
plot_overlap_disc_enough <- plot_ly(sample_overl_indic_specific, y = ~fair_enough_score, type = "box", name = "FAIR Enough") |>
  layout(title = "Selected data sets with overlapping indicators (discipline repositories)", yaxis = list(title = "Score in %"),showlegend = FALSE)

plot_overlap_disc_checker <- plot_ly(sample_overl_indic_specific, y = ~fair_checker_score, type = "box", name = "FAIR Checker") |>
  layout(title = "Selected data sets with overlapping indicators (discipline repositories)", yaxis = list(title = "Score in %"),showlegend = FALSE)
...
df_enough <- enough_cleaned |>
  select(c(ID,
           ...
           fair_enough_Gen2_FM_F3_data, fair_enough_Gen2_FM_F3_metadata, fair_enough_Gen2_FM_F4)) |>
  mutate(fair_enough_score = round((#enough_Gen2_FM_F1B_data + enough_Gen2_FM_F1A + enough_Gen2_FM_F1B_metadata +
                               #enough_Gen2_FM_F2A + enough_Gen2_FM_F2B+
                                 #enough_Gen2_FM_I1A_metadata+ enough_Gen2_FM_I1B_metadata+
                                 #enough_Gen2_FM_I1A_data+ enough_Gen2_FM_I1B_data+ enough_Gen2_FM_I2A+ enough_Gen2_FM_I2B+
                                # enough_Gen2_FM_I3+ enough_Gen2_FM_R1.1_metadata+ enough_Gen2_FM_R1.1_data
    fair_enough_Gen2_FM_F3_data+ fair_enough_Gen2_FM_F3_metadata+ fair_enough_Gen2_FM_F4)/3*100, digits = 2))
...
fuji_enough_long <-
  fuji_enough_selected_articles |> pivot_longer(cols = c("fuji_score", "fair_enough_score"), 
                                 names_to = "assessment_tool")
...
save(selected_articles, file = "./output/Rdata/selected_articles_df_2025_01_29.Rdata")
write.csv2(selected_articles, file = "./output/csv/selected_articles_df_2025_01_29.csv", row.names = F)
...
df_fuji_2 <- fuji_cleaned |>
  select(c(article, best_identifier, repository,repository_type, 
          ...
          # `FsF-F1-01D`, `FsF-F1-02D`, `FsF-F2-01M`, `FsF-I1-01M`, `FsF-I2-01M`,`FsF-I3-01M`, `FsF-R1.1-01M`, 
          `fuji_FsF_R1.2_01M`, `fuji_FsF_R1.3_01M`, `fuji_FsF_R1.3_02D`
  ))|>
  mutate(fuji_score = round(((#`FsF-F1-01D`+ `FsF-F1-02D`+ `FsF-F2-01M`+
                               # `FsF-I1-01M`+`FsF-I2-01M`+`FsF-I3-01M`+`FsF-R1.1-01M`+
                                `fuji_FsF_R1.2_01M`+ `fuji_FsF_R1.3_01M`+ 
                                  `fuji_FsF_R1.3_02D`))/ 8 *100, digits = 2))
# 3 + 2 + 3
...
df_checker <- checker_cleaned |>
  select(c(#fair_checker_f1, fair_checker_f2, fair_checker_i1, fair_checker_i2, fair_checker_i3, fair_checker_r1.1,
           ...
           fair_checker_R1.2, fair_checker_R1.3)) |>
  mutate(fair_checker_score = round((#checker_F1A+checker_F1B+checker_F2A+checker_F2B+
                                 # checker_I1+checker_I2+checker_I3+checker_R1.1+
                                  fair_checker_R1.2+ fair_checker_R1.3)/4*100, digits = 2))
...
fuji_checker_long <-
  fuji_checker_selected_articles |> pivot_longer(cols = c("fuji_score", "fair_checker_score"), 
                                 names_to = "assessment_tool")
...
df_enough_2 <- enough_cleaned |>
  select(c(ID,
           ...
           #enough_Gen2_FM_I1A_data, enough_Gen2_FM_I1B_data, enough_Gen2_FM_I2A, enough_Gen2_FM_I2B,
           #enough_Gen2_FM_I3, enough_Gen2_FM_R1.1_metadata, enough_Gen2_FM_R1.1_data,
           fair_enough_Gen2_FM_A1.1_data, fair_enough_Gen2_FM_A1.1_metadata, 
           fair_enough_Gen2_FM_A1.2_data, fair_enough_Gen2_FM_A1.2_metadata)) |>
  mutate(fair_enough_score = round((#enough_Gen2_FM_F1B_data + enough_Gen2_FM_F1B_metadata +enough_Gen2_FM_F1A + enough_Gen2_FM_F2A + enough_Gen2_FM_F2B+
    #enough_Gen2_FM_I1A_metadata+ enough_Gen2_FM_I1B_metadata+
    #enough_Gen2_FM_I1A_data+ enough_Gen2_FM_I1B_data+ enough_Gen2_FM_I2A+ enough_Gen2_FM_I2B+
    # enough_Gen2_FM_I3+ enough_Gen2_FM_R1.1_metadata+ enough_Gen2_FM_R1.1_data
    fair_enough_Gen2_FM_A1.1_data+ fair_enough_Gen2_FM_A1.1_metadata+ 
      fair_enough_Gen2_FM_A1.2_data+ fair_enough_Gen2_FM_A1.2_metadata)/4*100, digits = 2))
...
df_checker_2 <- checker_cleaned |>
  ...
  mutate(checker_score = round((#checker_F1A+checker_F1B+checker_F2A+checker_F2B+
    # checker_I1+checker_I2+checker_I3+checker_R1.1+
    fair_checker_A1.1 + fair_checker_A1.2)/4*100, digits = 2))

df_enough_checker <- bind_cols(df_enough_2,
                             df_checker_2)
enough_checker_selected_articles <- df_enough_checker[df_enough_checker$ID %in% 
                                                        fuji_checker_selected_articles$best_identifier,]

enough_checker_long <-
  enough_checker_selected_articles |> pivot_longer(cols = c("fair_enough_score", "checker_score"), 
                                                 names_to = "assessment_tool")
